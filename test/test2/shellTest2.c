...
main(void)
{
	...
	{ /*Ciclo principal*/
		...
		if(strlen(comando) > 0)
		{ /*Actuar solo si comando contiene algo y solo un entero*/
...
void separaArgs(void)
{
	...
	for(i = 0; i < (max_args - 1); i++)
		args[i] = NULL; /*Borrar argumento previo que pudiera existir*/
...
void listaDir(void)
{
	...
	else if(archs == 2) /*Si solo encontro los directorios . (actual) y .. (padre) consideraremos que el dir esta vacio a efectos practicos*/
...
void eco(void)
{
	...
	while(args[++k])
	{ /*recorrer argumento por argumentos mientra este tenga valor*/
		for(i=0; i < strlen(args[k]); i++)
		{ /*Para imprimir lo hare de caracter en caracter para interceptar el $ en caso de que este presente en cuanquier posicion*/
			if(args[k][i] != '$')
				printf("%c", args[k][i]);
